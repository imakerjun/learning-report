# AGENT.md — 헤르메스 에이전트 실행 계약서

이 문서는 **에이전트가 따르는 절차서**다. 챌린지 참여자가 디스코드에서
"이 템플릿으로 내 학습 리포트 만들고 배포해서 스크린샷 보여줘"라고 요청하면,
에이전트는 아래 절차를 **순서대로** 수행한다.

> 핵심 원칙: 에이전트는 **데이터만 깔고, 해석은 사람에게 돌려준다.**
> 합성 점수("학습 강도 0.72" 같은)를 지어내지 않는다. 화면의 모든 카드는
> 토픽 데이터에서 직접 계산된 사실(이름·증감 수치)뿐이다.
> 이것이 "Read LLM"이 아니라 "Read Less with LLM"이다.

---

## 절차 (6단계)

### 1. 저장소 클론
```bash
git clone https://github.com/imakerjun/learning-report.git
cd learning-report
```

### 2. 참여자 학습 데이터 분석 → 토픽 빈도 추출

참여자의 "학습 활동"이 남은 곳을 찾아 토픽별 등장 빈도를 센다. 출처 후보:
- 참여자의 작업 저장소 git 로그 (`git log --since`, 커밋 메시지·변경 파일 경로)
- 대화·메모·노트 텍스트 파일
- 학습 일지 / outbox 기록

추출 방법:
1. **이번주(current_week)** 와 **직전 N주(기본 4주)** 구간을 나눈다.
2. 참여자에게 의미 있는 **토픽 키워드 6~10개**를 정한다.
   - 키워드는 가능하면 **참여자 본인이 먼저 적게** 한다. 그 목록과 데이터에서
     실제로 많이 나온 키워드의 **차이 자체가 학습 신호**다.
3. 각 토픽의 `current`(이번주 빈도)와 `baseline_avg`(직전 N주 평균)를 센다.
4. 가능하면 주차별 `trend` 배열(최근 5개 값)도 만든다 → 스파크라인에 쓰임.

### 3. `data/report.json` 생성 (덮어쓰기)

아래 스키마로 `data/report.json`을 쓴다. index.html이 이 파일을 **자동으로**
먼저 읽는다(없으면 sample.json fallback). 형식은 `data/sample.json` 참고.

```json
{
  "meta": {
    "owner": "참여자 이름",
    "current_week": "2026-W23",
    "baseline_label": "직전 4주 평균",
    "baseline_weeks": 4,
    "generated_at": "2026-06-10",
    "source": "데이터를 어디서 뽑았는지 한 줄"
  },
  "topics": [
    { "name": "토픽명", "current": 정수, "baseline_avg": 정수, "trend": [수,수,수,수,수] }
  ],
  "readings": [
    { "lens": "에릭슨", "sees": "의도적 수련", "reading": "이 렌즈로 본 상태 한 단락", "feedback": "그래서 이렇게 해보라는 다음 행동 한 가지" },
    { "lens": "비요크", "sees": "다시 꺼내 쓰기", "reading": "이 렌즈로 본 상태 한 단락", "feedback": "그래서 이렇게 해보라는 다음 행동 한 가지" }
  ],
  "narrative": {
    "headline": "한 줄 요약",
    "strongest_axis": "지금 가장 깊게 파고 있는 것 한 줄",
    "themes": [{ "title": "...", "summary": "...", "points": ["...", "..."] }]
  }
}
```

**필드 산출 가이드**
- `current`: 이번주(current_week) 해당 토픽 등장 횟수.
- `baseline_avg`: **직전 N주간 주당 평균 등장 횟수.** (예: 직전 4주에 48번 → 12) 이 값이 화면의 점선 위치이자 비교 기준선이다.
- `trend`: 최근 5개 주차 값. 스파크라인에 쓰임.
- 화면 상단 4개 카드(새로 등장·사라짐·가장 큰 증가·가장 큰 감소)와 정렬은 모두
  `current` vs `baseline_avg`에서 **자동 계산**된다. 별도 필드 불필요.
- **가장 흥미로운 신호는 `current == 0 && baseline_avg > 0`(사라짐)** 이다. 본인이 가장 못 보는 자료다.
- `topics`만 있으면 동작한다. `readings`·`narrative`는 없으면 자동 생략(또는 계산된 분석 단락으로 대체)된다.

**`readings` 작성 규칙 (다중 관점 해석 + 피드백)** — 차트를 받아쓰지 말고 *읽고*, 거기서 *다음 행동*을 남겨라. 핵심:
1. **렌즈는 '말투'가 아니라 '무엇을 보는가'를 바꾼다.** 에릭슨은 *수련의 질*(많이 한 게 의도적이었나·피드백·약점 겨냥), 비요크는 *다시 꺼내 쓰기*(새로 연 것 말고 지난 걸 다시 꺼냈나·안 쓰면 사라진다)를 본다. 두 렌즈가 같은 사실을 다른 톤으로 반복하면 실패다. 보는 대상이 달라야 한다.
2. **실존 인물은 '원칙/렌즈'로만 빌리고 가짜 인용을 만들지 않는다.** "비요크가 말하길" ❌ → "다시 꺼내 쓰기 관점으로 보면" ⭕. 입에 없는 말을 넣지 않는다.
3. **`reading`은 상태(관찰), `feedback`은 다음 행동(처방)으로 분리한다.**
   - `reading`: 그 렌즈로 본 핵심 하나 + 질문 하나. 단정("~했다")보다 추측("~아닐까?")이 읽는 사람 머리를 깨운다.
   - `feedback`: "그래서 이렇게 해보라"는 **바로 할 수 있는 작은 행동 하나.** 추상 조언("더 열심히") 금지.
- 기본 렌즈는 **에릭슨(의도적 수련) + 비요크(다시 꺼내 쓰기)** 2개. 더 넣어도 되지만 3개 이하 권장. 렌즈는 화면 상단에 탭으로 바로 표시된다.

**⚠️ 길이 — 짧게. 길면 안 읽힌다. (가장 자주 어기는 규칙)**
화면에 한눈에 들어와야 한다. 아래 상한을 넘기지 마라:

| 필드 | 상한 |
|---|---|
| `reading` | **2문장, 약 90자 이내.** 관찰 1 + 질문 1 |
| `feedback` | **1문장, 약 45자 이내.** 행동 하나 |
| `narrative.headline` / `strongest_axis` | **1문장** |
| `narrative.themes[].title` | 한 구절(약 20자) |
| `narrative.themes[].summary` | **1문장, 약 50자** |
| `narrative.themes[].points` | 단어/짧은 구 **3~5개** |

- 미사여구·중복 설명·예시 나열 금지. **핵심 하나만.** `data/sample.json`의 readings·narrative가 길이 기준 예시다 — 그보다 길면 줄여라.

### 4. 정적 서버 실행
```bash
python3 -m http.server 8080
# (백그라운드로 띄우거나 별도 세션에서)
```

### 5. trycloudflare 터널 노출
```bash
cloudflared tunnel --url http://localhost:8080
# 출력된 https://<random>.trycloudflare.com 주소를 참여자에게 전달
# 최종 URL: https://<random>.trycloudflare.com/index.html
```
> `cloudflared`가 없으면 설치: `brew install cloudflared`

### 6. 스크린샷 캡쳐 후 전달
- 첫 화면에는 **예측 퀴즈 게이트**가 뜬다(사람이 차트를 보기 전에 먼저 맞혀보는 단계).
  스크린샷에는 차트가 필요하므로 **`?reveal=1`을 붙여** 게이트를 건너뛴 URL을 캡쳐한다:
  `index.html?reveal=1`
- 이 상태는 데이터 로드만으로 **클릭 없이 완성**되므로, 로드 후 ~1.2초 대기하면 막대 애니메이션이 정착된다.
- 참여자에게 전달할 때는 **퀴즈를 풀 수 있는 일반 URL**(`/index.html`)과
  **차트 스크린샷**(`?reveal=1`로 캡쳐한 것)을 함께 준다.
  → 사람은 URL로 들어가 직접 예측해보고, 스크린샷으로는 결과를 바로 확인한다.

---

## 체크리스트 (에이전트 self-check)

- [ ] `data/report.json`을 생성했는가 (sample.json을 수정한 게 아니라)
- [ ] 합성 점수를 지어내지 않았는가 (모든 값은 실제 빈도 카운트)
- [ ] `baseline_avg`가 합계가 아니라 **주당 평균**인가
- [ ] 토픽이 6~10개인가 (너무 많으면 화면이 흐려짐)
- [ ] http로 serve했는가 (file://로는 fetch가 막힌다)
- [ ] URL + 스크린샷 둘 다 전달했는가

---

## 설계 의도 (왜 이렇게 만들었는가)

- **변화량이 본체**: 빈도 순이 아니라 `|current − baseline_avg|` 순으로 정렬된다.
  본인이 가장 못 보는 자료는 "사라진 토픽"이다.
- **에이전트는 합성 결론을 만들지 않는다**: "학습 강도 0.72" 같은 지어낸 점수 대신,
  데이터에서 직접 읽히는 사실(어떤 토픽이 새로 떴고 무엇이 사라졌는가)만 보여준다.
  해석은 화면을 보는 사람의 몫이다.
- **단일 HTML · CDN 최소화**: clone → serve → screenshot 경로에서 실패 지점을 0에 가깝게.
  차트 라이브러리 없이 손으로 쓴 CSS + 인라인 SVG로 렌더한다.
