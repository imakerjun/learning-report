...
```json
{
  ...
  "readings": [
    { "lens": "에릭슨", "sees": "의도적 수련", "reading": "이 렌즈로 본 한 단락" },
    { "lens": "비요크", "sees": "인출·파지", "reading": "이 렌즈로 본 한 단락" }
  ],
  "narrative": {
    "headline": "한 줄 요약",
...
**`readings` 작성 규칙 (다중 관점 해석)** — 차트를 받아쓰지 말고 *읽어라*. 핵심 두 규칙:
1. **렌즈는 '말투'가 아니라 '무엇을 보는가'를 바꾼다.** 에릭슨은 *수련의 질*(많이 한 게 의도적이었나·피드백·약점 겨냥), 비요크는 *파지*(새로 연 것 말고 지난 걸 다시 꺼냈나·안 쓰면 사라진다)를 본다. 두 렌즈가 같은 사실을 다른 톤으로 반복하면 실패다. 보는 대상이 달라야 한다.
2. **실존 인물은 '원칙/렌즈'로만 빌리고 가짜 인용을 만들지 않는다.** "비요크가 말하길" ❌ → "인출 관점으로 보면" ⭕. 입에 없는 말을 넣지 않는다.
- 각 reading은 **사실 나열이 아니라 해석 + 질문**으로 끝낸다. 단정("~했다")보다 추측("~처럼 보인다·~아닌가")이 읽는 사람 머리를 깨운다.
- 기본 렌즈는 **에릭슨(의도적 수련) + 비요크(인출·파지)** 2개. 더 넣어도 되지만 3개 이하 권장.
